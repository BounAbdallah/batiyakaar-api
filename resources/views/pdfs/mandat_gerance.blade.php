...
<head>
    ...
    <title>Mandat de Gérance - {{ $bien->reference }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="content">
        ...
        <p>
            ...
            @if($bien->agence)
                l’agence <strong>{{ $bien->agence->raison_sociale }}</strong>
                @if($bien->agence->user)
                    représenté par son gérant monsieur <strong>{{ $bien->agence->user->prenom }}
                        {{ $bien->agence->user->nom }}</strong>
                    @if($bien->agence->user->cin)
                        titulaire de la CIN N° {{ $bien->agence->user->cin }}
                    @endif
                @endif
                administrateur de biens titulaire du NINEA : {{ $bien->agence->ninea }}
                et du RC : {{ $bien->agence->rccm }}.
            @else
        ...
        <p><strong>Article 15 :</strong><br>
            l’agence fera parvenir à monsieur {{ $bien->bailleur->user->prenom ?? '' }}
            {{ $bien->bailleur->user->nom ?? '' }} à la fin de chaque mois,
        ...
        <p><strong>APPLICATION</strong><br>
            ...
            Un (1) exemplaire pour l’agence {{ $bien->agence->raison_sociale ?? '' }}<br>
            Un (1) exemplaire pour monsieur {{ $bien->bailleur->user->prenom ?? '' }}
            {{ $bien->bailleur->user->nom ?? '' }}
        </p>
    ...
    <div class="agency-footer">
        @if($bien->agence)
            <strong>{{ $bien->agence->raison_sociale }}</strong><br>
            {{ $bien->agence->adresse }}<br>
            {{ $bien->agence->user->telephone ?? '' }}<br>
            N.I.N.E.A : {{ $bien->agence->ninea }} / RC : {{ $bien->agence->rccm }}
        @endif
    </div>
